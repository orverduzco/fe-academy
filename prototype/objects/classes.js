...
/**
 * CASE 2: If classes are actually functions, can they be called?
*/
function canAClassBeCalled () {
  class Animal {
    eat () {
      return `I'm eating.`
    }
  ...
  let resultOfClassCall = ''

  try {
    resultOfClassCall = Animal()
  } catch (error) {
    resultOfClassCall = error
  }

  // log result
  const logger = new Logger()
  logger.statements = [
    `Result of calling the class: ${resultOfClassCall}`
  ]
